"""
Abstract factory: It relies on composition. Objects are created in implemented methods exposed in the factory
interface.
"""
...
if 1:
    ...
    Although the Factory Pattern makes sure that all stores adhere to the proper procedures (since these are defined in 
    the abstract pizza class) it is not possible to ensure that stores use the proper components (in this case pizza 
    ingredients). Furthermore, some components are to be used with some stores, while others are not: 
    For example, Chicago uses plum tomato sauce, while NY uses marinara sauce.

    The solution to this, is to build factories -one for each region- that will be used to create the 
    components/ingredients. These ingredient factories will all extend -or implement- 
    the PizzaIngredientFactory abstract class or interface. Then, we will need to implement some ingredient classes to 
    be used with the factories and finally, all of this will need to be integrated into our PizzaStore.
